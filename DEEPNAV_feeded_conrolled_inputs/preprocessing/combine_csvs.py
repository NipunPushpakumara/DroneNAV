...
with PdfPages(all_flights_pdf) as pdf:
    ...
    for flight_number, flight_name in enumerate(flight_names):
        ...
        latest_starter = max([messages["imu"]["timestamps"][0], messages["baro"]["timestamps"][0],
                              messages["mag"]["timestamps"][0],messages["controls"]['timestamps'][0]])
        kfStartIdx = 0
        ...
        plot_signal(combined_data,signal_num=-2,y_label="East Position (m)",title=flight_name)
        plot_signal(combined_data,signal_num=-3,y_label="North Position (m)",title=flight_name)
